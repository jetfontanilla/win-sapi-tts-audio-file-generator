...
import csv
import configparser
from tts.sapi import Sapi
from tts.flags import SpeechVoiceSpeakFlags
...
for row in reader:
    ...
    sapi_flags = SpeechVoiceSpeakFlags.IsXML.value if cols > 2 else SpeechVoiceSpeakFlags.Default.value
    if sapi_input != '':
